...
def run_carla_client(args):
    ...
    vehicles_num = [30, 60, 50, 80, 120, 100, 100, 120, 110, 100, 80, 70, 70, 80, 100] 
                ...
                if frame > 199:
                    if args.save_images_to_disk:
                        for name, measurement in sensor_data.items():
                            filename = args.out_filename_format.format(episode, name, frame-200)
                            measurement.save_to_disk(filename)
        
                    player_measurements = measurements.player_measurements
                    with open("/data/khoshhal/Dataset/episode_{:0>4d}".format(episode) + ".txt", 'a') as myfile:
